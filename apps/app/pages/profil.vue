...
<template>
  <div class="mx-auto w-full max-w-3xl">
    ...
    <header>
      ...
      <p class="mt-2 max-w-xl text-sm leading-relaxed text-ink-500 sm:text-base">
        Tes informations réelles, la matière première de tous tes CV. Plus ton profil est complet,
        plus tes CV seront pertinents.
      </p>

      <div v-if="profile" class="mt-4 flex flex-wrap gap-2">
        <span
          class="inline-flex items-center gap-1.5 rounded-full bg-surface px-3 py-1 text-sm font-medium text-ink-600 shadow-xs ring-1 ring-border"
        >
          <Briefcase class="h-4 w-4 text-brand-600" :stroke-width="1.75" aria-hidden="true" />
          {{ expCount }} {{ expCount > 1 ? 'expériences' : 'expérience' }}
        </span>
        <span
          class="inline-flex items-center gap-1.5 rounded-full bg-surface px-3 py-1 text-sm font-medium text-ink-600 shadow-xs ring-1 ring-border"
        >
          <Wrench class="h-4 w-4 text-brand-600" :stroke-width="1.75" aria-hidden="true" />
          {{ skillCount }} {{ skillCount > 1 ? 'compétences' : 'compétence' }}
        </span>
      </div>
    </header>
    ...
    <div v-else class="mt-8 space-y-10">
      ...
      <section class="space-y-4" aria-labelledby="section-entete">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 id="section-entete" class="text-lg font-semibold text-ink-900">En-tête du profil</h2>
            <p class="text-sm text-ink-500">Ton intitulé et ton accroche, repris en haut de tes CV.</p>
          </div>
        </div>
      ...
      <section class="space-y-4" aria-labelledby="section-experiences">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 id="section-experiences" class="text-lg font-semibold text-ink-900">Expériences</h2>
        ...
        <ul v-if="profile?.experiences.length" class="space-y-3">
          ...
          >
            <div class="min-w-0">
              ...
              <p v-if="exp.description" class="mt-2 text-sm leading-relaxed text-ink-600 line-clamp-3">
                {{ exp.description }}
              </p>
      ...
      <section class="space-y-4" aria-labelledby="section-competences">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 id="section-competences" class="text-lg font-semibold text-ink-900">Compétences</h2>
              <p class="text-sm text-ink-500">
                Uniquement tes compétences réelles : elles font foi pour la génération de tes CV.
              </p>
            </div>
          </div>
          <UiButton v-if="!showSkillForm" variant="secondary" size="sm" @click="showSkillForm = true">
            <Plus class="h-4 w-4" :stroke-width="2" aria-hidden="true" />
          ...
          <form class="space-y-4" @submit.prevent="addSkill">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              <div class="flex flex-col gap-1.5">
                <label :for="levelSelectId" class="text-sm font-medium text-ink-700">Niveau</label>
                <select
                  :id="levelSelectId"
          ...
          >
            ...
            <div v-if="importResult.experiences.length" class="space-y-1.5">
              <p class="text-sm font-semibold text-ink-700">
                ...
                {{ importResult.experiences.length > 1 ? 'expériences détectées' : 'expérience détectée' }}
              </p>
              <ul class="space-y-1">
                <li
                ...
                >
                  <Briefcase class="h-3.5 w-3.5 shrink-0 text-brand-500" :stroke-width="1.75" aria-hidden="true" />
                  {{ e.title }} — {{ e.company }}
                </li>
              </ul>
            </div>
